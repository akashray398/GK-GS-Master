...
@AndroidEntryPoint
class StudyMaterialFragment : Fragment(R.layout.fragment_booklet_list) {
        ...
        viewModel.booklets.observe(viewLifecycleOwner) { resource ->
            when (resource) {
                ...
                is Resource.Success -> {
                    ...
                    val filtered = resource.data?.filter { it.type == BookletType.ORIGINAL_BOOKLET } ?: emptyList()
                    adapter.submitList(filtered)
                    println("Study Material: Displaying ${filtered.size} booklets")
                }
                is Resource.Error -> {
                    binding.progressBar.visibility = View.GONE
                    println("Study Material: Error ${resource.message}")
                }
            }
        }
    }
